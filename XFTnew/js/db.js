  ...
  return {
    ...
    parseWhereItem: function(key, val){
      if (isObject(val)) { // {id: {'<': 10, '>': 1}}
        ...
        //对字符串类型字段采用模糊匹配
        Iif (C('db_like_fields').indexOf(key) > -1) {
      ...
      if (isString(val[0])) {
        ...
        if (/^(=|!=|>|>=|<|<=)$/.test(val0)) { // 比较运算
          ...
          if (isArray(val[1])) { //多个like
            ...
            var self = this;
            Eif (likesLogic.indexOf(likeLogic) > -1) {
        ...
        }else Eif(val0 === 'BETWEEN'){ // BETWEEN运算
    ...
    select: function(options){
      ...
      var self = this;
      Iif (!isEmpty(cache) && C('db_cache_on')) {
        //内存缓存
        if (cache.type === '') {
